...
These kernels suffer from an issue that two input vectors cannot be allocated two different DDR channels exclusively in SYCL compiler in USM memory model. Once addressed, the performances of these kernels (except dsdot and sddot) are expected to be near-peak.
