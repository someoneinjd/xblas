...
void test(oneapi::mkl::transpose trans, oneapi::mkl::uplo upper_lower,
          int64_t n, int64_t k, Treal alpha, int64_t lda, Treal beta, int64_t ldc) {
...
int main() {
    ...
    float alpha = 2.0;
    float beta = 3.0;
    test<std::complex<float>, double>(oneapi::mkl::transpose::N, oneapi::mkl::uplo::U, n, k, alpha, lda, beta, ldc);
#else
