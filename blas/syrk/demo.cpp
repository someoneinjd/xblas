...
template <typename T>
void test(oneapi::mkl::transpose trans, oneapi::mkl::uplo upper_lower,
          int64_t n, int64_t k, T alpha, int64_t lda, T beta, int64_t ldc) {
    ...
    sycl::event e = t2sp::blas::row_major::syrk(q_device, trans, n, k, alpha, a.data(), lda,
                                                beta, c.data(), ldc);
    e.wait();
    ...
    matrix_multiply_statistics<T>(n, k, exec_time, total_flops, total_bytes);
...
int main() {
    ...
    float alpha = 2.0f;
    float beta  = 3.0f;
    test<float>(oneapi::mkl::transpose::N, oneapi::mkl::uplo::U, n, k, alpha, lda, beta, ldc);
    ...
    double alpha = 2.0f;
    double beta = 3.0f;
    test<double>(oneapi::mkl::transpose::N, oneapi::mkl::uplo::U, n, k, alpha, lda, beta, ldc);
    ...
    test<std::complex<float>>(oneapi::mkl::transpose::N, oneapi::mkl::uplo::U, n, k, alpha, lda, beta, ldc);
#else
